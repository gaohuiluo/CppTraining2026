...
#### `const int* p` —— 指向常量的指针（值不可改，指针可移动）

| 场景 | 说明 |
|---|---|
| **函数参数防止修改** | 最常见用法：`void print(const int* p)`，告知调用方函数不会修改数据 |
| **遍历只读数据** | 遍历数组时 `p++` 可行，但 `*p = ...` 编译报错 |
| **C 风格字符串** | `const char* str = "hello";`，字符串字面量本身不可变 |
| **观察者模式** | 持有对其他对象数据的只读引用，避免意外修改 |
...
#### `int* const q` —— 常量指针（指针本身不可改指向，值可改）

| 场景 | 说明 |
|---|---|
| **固定缓冲区 / 寄存器映射** | 指向硬件寄存器或固定内存地址，指针一旦绑定不能偏移 |
| **类成员中的固定资源** | 如日志输出流、配置块，绑定后不再切换 |
| **防止意外重定向** | 误写 `q = &other` 会被编译器直接拦截 |
| **引用语义的底层模拟** | 引用本质上就是编译器自动管理的 `int* const` |

```cpp
class Logger {
    int* const logBuffer;  // 绑定后不可切换缓冲区
public:
    Logger(int* buf) : logBuffer(buf) {}
    void write(int val) { *logBuffer = val; }  // ✓ 可以修改值
    // logBuffer = other;  // ✗ 不能改指向
};
```

#### `const int* const r` —— 常量指针指向常量（双重不可变）

| 场景 | 说明 |
|---|---|
| **配置常量** | 指向全局配置或常量表，既不能切换表也不能改内容 |
| **函数内部固定常量引用** | 在函数内锁定某个常量，杜绝任何修改 |
| **安全 API 设计** | 暴露只读句柄，调用方既不能改内容也不能替换目标 |
| **等价于 `const int&`** | 语义上非常接近，只是用指针实现 |

```cpp
const int VERSION_MAJOR = 2;

void showVersion() {
    const int* const v = &VERSION_MAJOR;  // 锁定：不切换、不修改
    printf("Version: %d", *v);
}
```
